...
const I18n = {
    ...
    data: {
        en: {
            ...
            "sett.tutorial": "Replay Tutorial", "sett.logout": "Logout", "sett.delete": "Delete Account",
            ...
            "lbl.goal": "Goal", "lbl.lvl": "Level", "lbl.env": "Environment",
            "chal.rules": "Calculation Rules:\\n\\n• Volume: Total sum of (Sets × Reps × Weight).\\n• Reps: Bodyweight exercises only.\\n• Time: Minutes in cardio exercises.\\n• Progress updates after finishing session.",
            ...
            "stat.title": "Weekly Progress", "stat.vol": "Volume (kg)",
            "chal.title": "Active Challenge", "chal.rew": "Reward", "chal.empty": "No active challenge", "chal.comp": "Completed!",
            "chal.vol.desc": "Sum of all weight lifted (in kg) during your workouts this week.",
            "chal.sess.desc": "Complete training sessions to build your habit.",
            "chal.reps.desc": "Complete bodyweight repetitions to build endurance.",
            "chal.time.desc": "Accumulate active time in cardio or hold exercises.",
            "chal.prog": "Progress", "chal.comp_pct": "completed",
            "chal.msg.start": "Let's go! Your goal is {{target}} {{unit}} this week. Each rep counts. You are ready!",
            "chal.msg.prog": "Great job! You've done {{current}} {{unit}}. Keep it up!",
            "chal.msg.behind": "A bit slow? Why not add more reps or intensity today?",
            "chal.msg.done": "Excellent! You reached {{target}} {{unit}}. Your strength is improving!",
            "stat.reps": "Reps", "stat.time": "Min",
            "rec.title": "Recovery Advisor", "rec.tip1": "Great effort! Take a rest day properly.", "rec.tip2": "Light session. You can train again tomorrow.",
            "rec.tip3": "High intensity detected. Focus on sleep and hydration.",
            "rec.close": "Got it",
            "gm.pts": "pts", "gm.lvl": "Lvl"
        },
        es: {
            ...
            "sett.tutorial": "Repetir Tutorial", "sett.logout": "Cerrar Sesión", "sett.delete": "Eliminar Cuenta",
            ...
            "lbl.goal": "Objetivo", "lbl.lvl": "Nivel", "lbl.env": "Entorno",
            "chal.rules": "Reglas de Cálculo:\\n\\n• Volumen: Suma total de (Series × Reps × Peso).\\n• Reps: Solo ejercicios de peso corporal.\\n• Tiempo: Minutos en ejercicios de cardio.\\n• El progreso se actualiza al finalizar la sesión.",
            ...
            "stat.title": "Progreso Semanal", "stat.vol": "Volumen (kg)",
            "chal.title": "Desafío Activo", "chal.rew": "Recompensa", "chal.empty": "Sin desafío activo", "chal.comp": "¡Completado!",
            "chal.vol.desc": "Suma todo el peso levantado (en kg) durante tus entrenamientos de esta semana.",
            "chal.sess.desc": "Completa sesiones de entrenamiento para crear el hábito.",
            "chal.reps.desc": "Realiza repeticiones de peso corporal para aumentar resistencia.",
            "chal.time.desc": "Acumula tiempo activo en cardio o ejercicios de resistencia.",
            "chal.prog": "Progreso", "chal.comp_pct": "completado",
            "chal.msg.start": "¡Vamos con todo! Tu objetivo es {{target}} {{unit}} esta semana. Cada repetición cuenta. ¡Estás listo!",
            "chal.msg.prog": "¡Estás haciendo un gran trabajo! Llevas {{current}} {{unit}}. ¡No pares ahora!",
            "chal.msg.behind": "Veo que vas un poco atrás. ¿Por qué no pruebas con más repeticiones hoy?",
            "chal.msg.done": "¡Excelente! Has completado {{target}} {{unit}}. ¡Tu fuerza mejora constantemente!",
            "stat.reps": "Reps", "stat.time": "Min",
            "chal.vol.desc": "Suma todo el peso levantado (en kg) durante tus entrenamientos de esta semana.",
            "chal.sess.desc": "Completa sesiones de entrenamiento para crear el hábito.",
            "chal.prog": "Progreso", "chal.comp_pct": "completado",
            "rec.title": "Asesor de Recuperación", "rec.tip1": "¡Gran esfuerzo! Descansa bien hoy.", "rec.tip2": "Sesión ligera. Puedes entrenar mañana.",
            "rec.tip3": "Alta intensidad. Prioriza sueño y agua.",
            "rec.close": "Entendido",
            "gm.pts": "pts", "gm.lvl": "Nvl"
        }
    },
...
const CATALOG = {
    ...
    "burpee": { muscle: "legs", mode: "body", tier: 2, en: "Burpees", es: "Burpees" },
    "mountain_climber": { muscle: "core", mode: "time", tier: 2, en: "Mountain Climbers", es: "Escaladores" },

    // Gym Exclusive (Tier 3)
    "squat_bar": { muscle: "legs", mode: "load", tier: 3, req: ["gym"], en: "Barbell Squat", es: "Sentadilla con Barra" },
    "bench_bar": { muscle: "push", mode: "load", tier: 3, req: ["gym"], en: "Bench Press", es: "Press de Banca" },
    "deadlift_bar": { muscle: "pull", mode: "load", tier: 3, req: ["gym"], en: "Deadlift", es: "Peso Muerto" },
    "lat_pulldown": { muscle: "pull", mode: "load", tier: 2, req: ["gym"], en: "Lat Pulldown", es: "Jalón al Pecho" }
};
...
const Trainer = {
    ...
    getSmartTargets: (user, exId, mode) => {
        ...
        if (mode === 'load') {
            ...
            if (rpe === '1') { targets.val1 += 5; tip = 'tip.inc'; }
            else if (rpe === '4') { targets.val1 = Math.max(5, targets.val1 - 5); }
        } else {
            // Time logic
            if (rpe === '1') { targets.val1 += 30; tip = 'tip.inc'; }
        }
    ...
    generateRoutine: (user, env = 'gym') => {
        ...
        // Smart Tools: If Gym, assume all tools. If Home, use settings.
        const tools = env === 'gym' ? ['gym', 'bar', 'dumbbells', 'rope'] : (user.settings.equip || []);

        // SMART ADJUSTMENT: Use AI Level
        let effectiveLevel = EvoAI.getSmartLevel(user);

        // Dynamic Level Adjustment Fallback (Legacy)
        if (lvl === 'beginner' && (user.points || 0) > 1000) effectiveLevel = 'intermediate';
        if (lvl === 'intermediate' && (user.points || 0) > 3000) effectiveLevel = 'advanced';
        ...
        return schema.map(m => {
            ...
            const candidates = Object.keys(CATALOG).filter(k => {
                ...
                // Tier Filter with Effective Level
                if (effectiveLevel === 'beginner' && item.tier > 2) return false;
                if (effectiveLevel === 'advanced' && item.tier < 2) return false; // Strict advanced filter
...
// --- 6. EVO AI (Machine Learning Lite) ---
const EvoAI = {
    // Continuous Learning from User Performance
    learn: (user, session) => {
        if (!user.ai) user.ai = { confidence: 0, strengthIndex: 1.0, lastSession: 0 };

        // 1. Analyze RPE (Feedback Loop)
        // Low RPE (Easy) -> Increase Strength Index
        // High RPE (Hard) -> Decrease Strength Index
        // Target RPE is 2.5 - 3.0
        const rpe = session.rpeAvg || 3;
        let adjustment = 1.0;

        if (rpe < 2.0) adjustment = 1.05; // Too easy, +5%
        else if (rpe < 3.0) adjustment = 1.02; // Good, +2%
        else if (rpe > 4.0) adjustment = 0.95; // Too hard, -5%

        user.ai.strengthIndex *= adjustment;

        // Clamp (Safeguard)
        user.ai.strengthIndex = Math.max(0.7, Math.min(3.0, user.ai.strengthIndex));

        // 2. Consistency Bonus
        const now = Date.now();
        const daysSince = (now - user.ai.lastSession) / (1000 * 60 * 60 * 24);
        if (daysSince < 3) user.ai.confidence += 1;
        else if (daysSince > 7) user.ai.confidence = Math.max(0, user.ai.confidence - 2);

        user.ai.lastSession = now;
    },

    // Dynamic Level Classification
    getSmartLevel: (user) => {
        const baseLvl = user.settings.level;
        if (!user.ai) return baseLvl;

        const score = user.ai.strengthIndex + (user.ai.confidence * 0.05);

        if (baseLvl === 'beginner' && score > 1.5) return 'intermediate';
        if (baseLvl === 'intermediate' && score > 2.2) return 'advanced';

        // Downgrade protection (optional, maybe discourages users)
        return baseLvl;
    },

    // Adaptive Challenge Targets
    adjustTarget: (user, baseTarget) => {
        if (!user.ai) return baseTarget;
        return Math.floor(baseTarget * user.ai.strengthIndex);
    }
};

// --- 7. VIEW CONTROLLER (UI) ---
const UI = {
...
window.App = {
    ...
    init: async () => {
        ...
        document.querySelectorAll('.rpe-btn').forEach(b => {
            b.onclick = () => App.setRPE(b.dataset.v);
        });
    },
    ...
    loadHub: () => {
        UI.goto('v-hub');
        UI.text('user-name', `${App.user.name} (Lvl ${App.user.level || 1})`);
        UI.text('sess-count', App.user.points || 0);
        document.querySelector('[data-bind="hub.sess"]').textContent = I18n.t('gm.pts');
        ...
        const envKey = App.user.settings.env || 'gym';
        const envFull = I18n.t(`env.${envKey}`);

        const combinedText = `${goalFull} - ${lvlFull} - ${envFull}`;
    ...
    updateSettings: async () => {
        ...
        if (u.settings.env !== env) { u.settings.env = env; changed = true; }
    ...
    register: async () => {
        ...
        const newUser = {
            ...
            points: 0, level: 1, challenge: null,
            settings: { lang: 'es', theme: 'dark', goal: 'hypertrophy', level: 'intermediate', rTime: 90, env: 'gym', equip: undefined }
        };
    ...
    generate: (env = null) => {
        ...
        const currentEnv = env || (sel ? sel.value : (App.user.settings.env || 'gym'));
    ...
    toggleSessTimer: () => {
        const el = document.getElementById('inp-1');
        const btn = document.getElementById('btn-timer-toggle');

        if (App.sess.workTimer) {
            // Pause
            clearInterval(App.sess.workTimer);
            App.sess.workTimer = null;
            btn.textContent = '▶';
        } else {
            // Start
            btn.textContent = '⏸';
            App.sess.workTimer = setInterval(() => {
    ...
    renderSet: () => {
        ...
        const bDec = document.getElementById('btn-t-dec');
        const bInc = document.getElementById('btn-t-inc');
        const bPlay = document.getElementById('btn-timer-toggle');

        bDec.style.display = 'none';
        bInc.style.display = 'none';
        bPlay.style.display = 'none';
        if (App.sess.workTimer) clearInterval(App.sess.workTimer);
        App.sess.workTimer = null;
        if (bPlay) bPlay.textContent = '▶';
        inp1.readOnly = false;
    ...
    finishSet: async () => {
        ...
        if (App.sess.set < ex.sets) {
            ...
            const rpeVal = parseInt(ex.rpe);
            App.sess.recommendation = null;

            if (rpeVal === 1) { // Too Easy
                let inc = 0;
                let unit = '';
                if (info.mode === 'time') { inc = 30; unit = 's'; }
                else if (info.mode === 'load') { inc = 2.5; unit = 'kg'; }
                else { inc = 5; unit = ' reps'; }

                App.sess.recommendation = {
                    type: 'increase',
                    val: inc,
                    msg: I18n.lang === 'es' ? `¿Muy Fácil? +${inc}${unit}` : `Too Easy? +${inc}${unit}`
                };
            } else if (rpeVal === 4) { // Failure
                let dec = 0;
                let unit = '';
                if (info.mode === 'time') { dec = 15; unit = 's'; }
                else if (info.mode === 'load') { dec = 5; unit = 'kg'; }
                else { dec = 5; unit = ' reps'; }

                App.sess.recommendation = {
                    type: 'decrease',
                    val: dec,
                    msg: I18n.lang === 'es' ? `¿Duro? -${dec}${unit}` : `Hard? -${dec}${unit}`
                };
            }

            // Fatigue Check
            const recentRPE = App.activeWorkout.slice(0, App.sess.idx + 1).map(e => parseInt(e.rpe || 2));
            const fatigue = recentRPE.slice(-3).filter(r => r >= 3).length;
            if (fatigue >= 3) {
                UI.toast(I18n.lang === 'es' ? 'Alta fatiga detectada. Cuida la técnica.' : 'High fatigue detected. Watch form.', 'warning');
            }
    ...
    completeSession: async () => {
        const totalVol = App.activeWorkout.reduce((acc, ex) => {
            return acc + (ex.sets * ex.target.val1 * (ex.target.val2 || 1));
        }, 0);

        // Track additional metrics
        const totalReps = App.activeWorkout.reduce((acc, ex) => {
            const info = CATALOG[ex.id];
            if (info && info.mode === 'body') return acc + (ex.sets * ex.target.val1);
            if (info && info.mode === 'load') return acc + (ex.sets * ex.target.val1); // Reps is val1 in 'load' mode usually? Wait.
            // In 'load' mode: val1=Reps, val2=KG. See renderSet logic.
            // renderSet: l1 (reps or kg). if load: inp1=kg(val2), inp2=reps(val1).
            // Wait, logic in lines 1030-1033:
            // if load: inp1=val2(KG), inp2=val1(Reps).
            // if body: inp1=val1(Reps), inp2=val2(KG/+KG).
            // So for 'load', reps is val1. Correct.
            return acc;
        }, 0);

        const totalTime = App.activeWorkout.reduce((acc, ex) => {
            const info = CATALOG[ex.id];
            if (info && info.mode === 'time') return acc + (ex.sets * ex.target.val1);
            return acc;
        }, 0); // in seconds

        const rpeAvg = App.activeWorkout.reduce((acc, ex) => acc + parseInt(ex.rpe || 2), 0) / App.activeWorkout.length;

        // Points
        App.user.points = (App.user.points || 0) + 100 + Math.floor(totalVol / 1000);
        App.user.level = Math.floor(App.user.points / 1000) + 1;

        // Challenge Check
        if (App.user.challenge && !App.user.challenge.completed) {
            let progInc = 0;
            switch (App.user.challenge.type) {
                case 'volume': progInc = totalVol; break;
                case 'reps': progInc = totalReps; break;
                case 'time': progInc = Math.floor(totalTime / 60); break; // Minutes
                case 'sessions': progInc = 1; break;
            }

            App.user.challenge.progress += progInc;

            if (App.user.challenge.progress >= App.user.challenge.target) {
                App.user.challenge.completed = true;
                App.user.points += App.user.challenge.reward;
                UI.toast(`Challenge Complete! +${App.user.challenge.reward} pts`, 'success');
            }
        } else if (!App.user.challenge) {
            App.generateChallenge();
        }

        App.user.history.push({ date: Date.now(), routine: App.activeWorkout, volume: totalVol, rpeAvg });

        // EvoAI Learning Step
        EvoAI.learn(App.user, { rpeAvg, volume: totalVol });

        await Store.saveUser(App.user);

        UI.toast('Workout Saved! AI Updated.', 'success');

        // Recovery Modal
        let recMsg = 'rec.tip1';
        if (rpeAvg < 2.5) recMsg = 'rec.tip2';
        if (rpeAvg > 3.5) recMsg = 'rec.tip3';

        alert(`${I18n.t('rec.title')}\n\n${I18n.t(recMsg)}\n\nAvg RPE: ${rpeAvg.toFixed(1)}`);

        App.loadHub();
    },

    renderStats: () => {
        // Mock implementation of rendering stats to a container
        const c = document.getElementById('stats-container');
        if (!c) return;
        c.innerHTML = '';

        // Simple Bar Chart for Volume
        const hist = [...App.user.history].reverse().slice(0, 5).reverse();
        if (hist.length === 0) { c.innerHTML = '<small>No data yet</small>'; return; }

        const maxVol = Math.max(...hist.map(h => h.volume || 1000));
        ...
        hist.forEach(h => {
            ...
            const hVol = h.volume || 0;
            const hPct = (hVol / maxVol) * 100;
    ...
    generateChallenge: () => {
        ...
        const eq = App.user.settings.equip || [];
        const lvl = App.user.settings.level;

        // Logic:
        // 1. Gym/Advanced Weights -> High Volume
        if (lvl === 'advanced' && (eq.includes('bar') || eq.includes('dumbbells'))) {
            type = 'volume';
            target = 5000;
            reward = 1000;
        }
        // 2. Home Weights -> Mod Volume
        else if (eq.includes('dumbbells') || eq.includes('bar')) {
            type = 'volume';
            target = 1000;
            reward = 500;
        }
        // 3. Cardio/Minimal -> Time/Distance (Time for now)
        else if (eq.includes('rope')) {
            type = 'time'; // minutes
            target = 100;
            reward = 500;
        }
        // 4. Default -> Reps (Bodyweight)
        else {
            type = 'reps';
            target = 2000;
            reward = 500;
        ...
        // AI Adjustment
        target = EvoAI.adjustTarget(App.user, target);
    ...
    renderChallenge: () => {
        const c = document.getElementById('chal-container');
        if (!c) return;

        const ch = App.user.challenge;
        if (!ch) {
            App.generateChallenge();
            return App.renderChallenge();
        }
        ...
        if (ch.type === 'volume') {
            title = I18n.lang === 'es' ? `Levanta ${ch.target} kg` : `Lift ${ch.target} kg`;
            desc = I18n.t('chal.vol.desc');
        } else if (ch.type === 'reps') {
            title = I18n.lang === 'es' ? `Realiza ${ch.target} reps` : `Complete ${ch.target} reps`;
            desc = I18n.t('chal.reps.desc');
        } else if (ch.type === 'time') {
            title = I18n.lang === 'es' ? `Acumula ${ch.target} min` : `Accumulate ${ch.target} min`;
            desc = I18n.t('chal.time.desc');
        } else {
            title = I18n.lang === 'es' ? `Completa ${ch.target} sesiones` : `Complete ${ch.target} sessions`;
            desc = I18n.t('chal.sess.desc');
        }

        // Coach Logic
        const unit = getUnit(ch.type);
        let msgKey = 'chal.msg.prog';

        if (ch.completed) {
            msgKey = 'chal.msg.done';
        } else if (ch.progress === 0) {
            msgKey = 'chal.msg.start';
        } else {
            // Check Pace
            if (!ch.startDate) ch.startDate = Date.now() - (1000 * 60 * 60 * 24); // fallback 1 day
            const days = Math.max(1, (Date.now() - ch.startDate) / (1000 * 60 * 60 * 24));
            const expected = (ch.target / 7) * days;

            if (ch.progress < expected * 0.8) {
                msgKey = 'chal.msg.behind';
            }
        }

        const coachMsg = I18n.t(msgKey)
            .replace('{{target}}', ch.target)
            .replace('{{current}}', ch.progress)
            .replace('{{unit}}', unit);
            ...
            <div style="background:rgba(255,255,255,0.05); padding:20px; border-radius:12px; border: 1px solid var(--border);">
                <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:16px;">
                    <div style="flex:1; padding-right:12px;">
                       <h3 style="margin:0 0 8px 0; font-size:1.1rem; line-height:1.3; color:var(--text);">${title}</h3>
                       <p style="margin:0; font-size:0.85rem; color:var(--text-muted); line-height:1.4;">
                           ${desc} <span onclick="alert(I18n.t('chal.rules'))" style="cursor:pointer; opacity:0.7; font-style:normal; margin-left:4px;">ⓘ</span>
                       </p>
                    </div>
                    <div style="text-align:right; min-width:80px;">
                       <div style="color:var(--primary); font-weight:bold; font-size:1.1rem; margin-bottom:4px;">+${ch.reward} pts</div>
                       <div style="font-size:0.7rem; color:var(--text-muted);">(${pct}% ${I18n.t('chal.comp_pct')})</div>
                    </div>
                </div>
                
                <div style="background:rgba(255,255,255,0.1); height:12px; border-radius:6px; margin-bottom:12px; overflow:hidden;">
                    <div style="background:var(--primary); height:100%; width:${pct}%; transition: width 0.5s ease;"></div>
                </div>

                <div style="background: rgba(0,0,0,0.2); padding: 10px; border-radius: 6px; margin-bottom: 12px; border-left: 3px solid var(--primary);">
                   <small style="color:var(--text); font-style:italic;">"${coachMsg}"</small>
                </div>
                
                <div style="display:flex; justify-content:space-between; font-size:0.9rem; font-weight:500; color:var(--text);">
                   <span>${I18n.t('chal.prog')}:</span>
                   <span>${ch.progress} ${ch.type === 'volume' ? 'kg' : ''} / ${ch.target} ${ch.type === 'volume' ? 'kg' : ''}</span>
                </div>
            </div>
        `;
    },
    ...
    doRest: () => {
        ...
        // Render Advisor
        const adv = document.getElementById('rest-advisor');
        if (App.sess.recommendation) {
            adv.innerHTML = `
                <button class="btn btn-outline animate-in" onclick="App.applyRec()" style="border-color:var(--primary); color:var(--primary);">
                    ${App.sess.recommendation.msg}
                </button>
             `;
        } else {
            adv.innerHTML = `<p style="opacity:0.6; margin:0;" data-bind="sess.recover">${I18n.t('sess.recover')}</p>`;
        }
    ...
    applyRec: () => {
        if (!App.sess.recommendation) return;
        const rec = App.sess.recommendation;
        const ex = App.activeWorkout[App.sess.idx];
        const info = CATALOG[ex.id];

        // Apply logic
        if (info.mode === 'load') {
            if (rec.type === 'increase') ex.target.val2 += rec.val;
            else ex.target.val2 = Math.max(0, ex.target.val2 - rec.val);
        } else {
            if (rec.type === 'increase') ex.target.val1 += rec.val;
            else ex.target.val1 = Math.max(0, ex.target.val1 - rec.val);
        }

        UI.toast(I18n.lang === 'es' ? 'Ajuste aplicado' : 'Adjustment applied', 'success');
        document.getElementById('rest-advisor').innerHTML = `<p style="color:var(--primary); font-weight:bold;">${I18n.lang === 'es' ? '¡Hecho!' : 'Done!'}</p>`;
        App.sess.recommendation = null;
    },
...
function getUnit(type) {
    ...
    if (type === 'time') return I18n.t('stat.time');
    if (type === 'reps') return I18n.t('stat.reps');
    return '';
}
